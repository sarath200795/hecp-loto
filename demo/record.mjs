...
const outFile = path.join(outDir, 'hecp-loto.mp4')
const silentFile = path.join(outDir, 'hecp-loto.silent.mp4')
...
const t0 = Date.now()
const rec = await startScreencast(page, path.join('/tmp', 'frames-hecp-loto'))
initAudio(path.join('/tmp', 'audio-hecp-loto'), t0)
...
if (res) {
  ...
  const wav = path.join('/tmp', 'audio-hecp-loto', 'narration.wav')
